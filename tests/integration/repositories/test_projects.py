...
def test_create_persists_project_and_generates_id(db_session: Session) -> None:
    ...
    created = repository.create(
        ProjectInput(name="Created Project", description="Created by repository.")
    )
    db_session.commit()
...
def test_replace_updates_persisted_project(db_session: Session) -> None:
    ...
    updated = repository.replace(
        project,
        ProjectInput(name="Updated Project", description="Updated description."),
    )
    db_session.commit()
...
def create_project(repository: ProjectRepository, session: Session, name: str):
    ...
    project = repository.create(
        ProjectInput(name=name, description=f"Description for {name}.")
    )
    session.commit()
