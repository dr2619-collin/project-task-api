...
Fixture lifetimes:
    ...
    One `pytest` command
    └── one PostgreSQL container and one SQLAlchemy Engine
        ├── each repository test gets a fresh database state and explicit
        │   SQLAlchemy Session
        └── each API router test gets a fresh database state and TestClient;
            the application creates SQLAlchemy Sessions for each operation

The container and engine use `scope="session"`, so they are shared for the
complete pytest command. The database reset, SQLAlchemy Session, and
TestClient use the default `scope="function"`, so each test function starts
with isolated state. These pytest fixture scopes are separate from the
SQLAlchemy `Session` object created for database work.
"""
...
def postgres_container() -> Generator[PostgresContainer, None, None]:
    ...
    with PostgresContainer(
        ...
        # Set this before creating the application's database infrastructure.
        # The application then connects to this temporary database rather than
        # the local development database named project_task.
...
def test_engine(postgres_container: PostgresContainer) -> Engine:
    ...
    # Build the same database infrastructure as the application, but with the
    # Testcontainers URL installed by the fixture above.
    return create_database(Settings()).engine
...
@pytest.fixture
def db_session(
    reset_database: None, test_engine: Engine
